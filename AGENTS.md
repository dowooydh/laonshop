...
3. **미확보 PG 스펙은 `NEEDS_PG_SPEC` 주석 + mock** — 구인증·빌링키·계좌이체·취소 API 등.
4. **결제수단**: 인증결제(카드 + 간편결제: 카카오·네이버·삼성페이)는 KSPAY **결제창에 통합**되어 고객이 선택. 계좌이체·정기결제·수기는 스펙 확정 후 실연동.
...
> 이 문서는 `AGENTS.md`와 동일 내용 미러 — 규칙을 바꾸면 **둘 다** 갱신한다.

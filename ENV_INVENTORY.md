...
> 작성: 2026-07-10 · 마지막 갱신: 2026-07-18 · 범위: **laonshop 저장소 1개(Next.js 15 웹앱)**. 모바일/EC2/CI는 이 저장소에 없음(아래 "해당 없음" 참조).
>
...
**요약:** 코드가 쓰는 변수 총 13종(+NODE_ENV 자동). 기존 운영 필수 6종은 로컬·Vercel(Prod+Preview) 모두 존재한다는 종전 기록을 유지한다. `KSPAY_STORE_KEY`/`KSPAY_API_KEY`/`KSPAY_REST_LIVE`/`KSPAY_WEBFEP_BASE`는 **KSNET 실연동 계약 전이라 의도적으로 미설정**이다. 수기결제는 API 키와 운영 스위치가 모두 없으면 UI·서버에서 차단하며 mock 승인 경로가 없다. 원클릭 빌링은 LAONPAY hosted flow와 opaque `paymentMethodId` 방식으로 준비하며, 신규 LAONPAY env 3종과 전용 DB 스키마가 모두 준비되기 전에는 fail-closed다. 고정 apex 복귀 URL과 쿠키 범위를 지키기 위해 이 3종은 **Vercel Production에만** 설정하고 Preview·Development에는 설정하지 않으며, 코드도 Production 이외 런타임에서는 외부 빌링 호출을 차단한다. LAONSHOP에는 카드 원문·KSNET `billingToken`·`pgapi`를 설정하거나 저장하지 않는다.
...
| **Vercel 대시보드** | 기존 배포용 env 6종 + 연동 준비 후 LAONPAY 빌링 env 3종(**Production scope만**, 암호화 저장) | vercel.com → `customorder/laonshop` → Settings → Environment Variables |
...
**연동 준비 중:** LAONPAY 호스팅 빌링 파트너 API. LAONSHOP은 등록 시작·고객/주문 원장·결과 표시만 담당하고, 카드 입력과 provider token vault·PG 호출은 LAONPAY가 담당한다. 환경변수와 DB 스키마가 미준비된 현재는 fail-closed이며 실 PG 연동 완료 상태가 아니다.
...
코드가 참조하는 계약 후 KSPAY 변수 4종과 LAONPAY 빌링 변수 3종을 **빈/주석 placeholder**로 관리함(값 없음): `KSPAY_STORE_KEY`, `KSPAY_API_KEY`, `KSPAY_REST_LIVE`, `KSPAY_WEBFEP_BASE`, `LAONPAY_BILLING_API_BASE`, `LAONPAY_PARTNER_KEY_ID`, `LAONPAY_PARTNER_PRIVATE_KEY`. `KSPAY_REST_LIVE`는 API 키가 미리 설정돼도 실제 승인이 나가지 않도록 하는 명시적 이중 가드다. LAONPAY 3종은 고정 운영 복귀 URL 때문에 Vercel **Production scope에만** 설정한다. 실값은 절대 커밋하지 않는다.
...
4. **원클릭 빌링 개발 시연·운영 전환** — 개발 시연 MID는 공용 테스트 MID `2999199999`로 결정했다. KSNET 공식 문서 콘솔에서 등록·조회·결제·취소·해지는 검증 완료했으나, 라온샵 계정·서버 연동 완료를 뜻하지 않는다. LAONPAY 호스팅 등록/API와 전용 LAONSHOP 빌링 DB 스키마를 먼저 준비한 뒤, 라온샵에는 `LAONPAY_BILLING_API_BASE`와 Ed25519 파트너 서명 env 2종만 설정한다. 카드 원문·KSNET `billingToken`·`pgapi`·공개 샘플 인증 문자열은 라온샵 코드·DB·Vercel에 두지 않는다. 고정 복귀 URL은 `https://laonshop.com/mypage/settings/billing/return`이며 query가 아닌 signed GET 상태 조회를 최종 근거로 사용한다. 서명 canonical은 POST의 소문자 UUID 멱등키까지 포함한 7줄이며 GET은 멱등키 줄과 header를 비운다. 등록/결제 응답 유실 시 같은 idempotency key·동일 본문의 reconciliation POST 1회만 허용하고 계속 `UNKNOWN`이면 신규 결제를 차단한다. 취소는 재POST하지 않고 전용 cancel-request signed GET으로 완료·반려를 확정한다. 공용 MID 외부 사용 정책과 별도 실 MID·권한은 정식 출시 전에 다시 확인한다. [LAONPAY API 준비·DB 스키마·파트너 키 발급·보안 검토]

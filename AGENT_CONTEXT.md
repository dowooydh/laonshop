...
> 마지막 정리: 2026-07-11. Claude Code auto-memory에서 현재 유효한 내용만 선별 이관한 요약이다. 제품·결제 규칙은 `AGENTS.md`, 구현 사실은 현재 코드와 git 기록을 우선한다.
...
- KSPAY 테스트 MID 결제창, 원클릭 빌링, 수기결제 흐름이 코드에 있다. WEBFEP 실승인은 `KSPAY_API_KEY`와 `KSPAY_REST_LIVE=1` 이중 가드이며, 실 MID·API 키·계약 상태는 운영 활성화 전에 다시 확인한다.

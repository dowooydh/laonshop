...
- 심사 화면 확인 계정 `laontest@laontest.com`에는 카드 등록 모달과 마스킹 카드 행을 브라우저 탭에서만 시연하는 UI가 노출된다. 고정 마스킹 예시만 사용하며 네트워크 요청·DB 저장·빌링키 발급·원클릭 결제 연결은 전혀 없고, 다른 계정에는 노출되지 않는다. 이는 아래 실연동 blocker를 우회하지 않는다.
...
1. 현재 blocker는 외부 서버용 전용 개발 `pgapi`의 안전한 발급·전달이다. 키가 오기 전에는 공식 문서 콘솔 검증과 기존 fail-closed 상태만 유지하며 mock 빌링을 복구하지 않는다.

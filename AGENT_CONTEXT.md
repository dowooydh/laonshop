...
- 사업자 footer의 대표자·주소·통신판매업신고번호는 ㈜커스텀오더 서류 기준으로 반영됐다. 신고번호는 `2025-성남분당A-0152`이며, 개인정보가 포함된 신고증 원본은 gitignore된 `reference/legal/`에만 보관하고 공개 자산·Git에는 포함하지 않는다.
- 공개 화면에 `더미`, `테스트`, `심사용` 같은 표현을 노출하지 않는다. 지정 심사 계정의 결제 시연 화면과 그 계정이 만든 시연 주문에는 실제 청구가 없음을 오인 없이 표시한다.

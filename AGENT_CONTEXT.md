...
> 마지막 정리: 2026-07-14. 제품·결제 규칙은 `AGENTS.md`, 구현 사실은 현재 코드와 git 기록을 우선한다.
...
- KSPAY 테스트 MID 인증결제창과 수기결제 흐름이 코드에 있다. 원클릭 빌링은 PG 빌링키가 아닌 mock이었던 경로를 제거했으며, KSNET 등록·승인·해지 계약과 카드 입력 보안 방식이 확정되기 전까지 신규 카드 등록과 원클릭 결제를 서버에서도 차단한다. 과거 mock 카드 레코드는 설정에서 삭제만 가능하다.
- 수기결제 WEBFEP 실승인은 `KSPAY_API_KEY`와 `KSPAY_REST_LIVE=1` 이중 가드이며, 실 MID·API 키·계약 상태는 운영 활성화 전에 다시 확인한다.
...
- KSPAY 승인과 수기결제처럼 거래나 카드 상태를 만드는 테스트는 사용자가 명시적으로 요청할 때만 한 번 수행한다. 비활성화된 빌링 등록·원클릭 결제는 실제 계약과 안전 연동 완료 전까지 테스트 명목으로도 다시 열지 않는다.

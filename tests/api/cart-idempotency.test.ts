...
test("기존 장바구니 내용과 nonce를 유지하며 복구된 상세컷 참조를 새 버전으로 올린다", () => {
  const originalStorage = Object.getOwnPropertyDescriptor(globalThis, "localStorage");
  const originalWindow = Object.getOwnPropertyDescriptor(globalThis, "window");
  const item = {
    productId: "p-legacy",
  ...
  try {
    ...
    assert.deepEqual(cart, [{ ...item, imageUrl: `${item.imageUrl}?v=20260715-editorial` }]);
...
test("잘못된 이미지 URL 마이그레이션 쓰기가 실패해도 장바구니와 nonce를 반환한다", () => {
  const originalStorage = Object.getOwnPropertyDescriptor(globalThis, "localStorage");
  const originalWindow = Object.getOwnPropertyDescriptor(globalThis, "window");
  const item = {

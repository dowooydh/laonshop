...
test("카드 등록은 hosted Action만 사용하며 라온샵에 카드 원문 입력·provider token 저장 경로가 없다", () => {
  ...
  const legacyModel = schema.slice(
    schema.indexOf("model ShopBillingCard"),
    schema.indexOf("model ShopBillingPaymentMethod"),
  );
  assert.match(legacyModel, /billingToken\s+String\s+@unique\s+@ignore/);
...
test("checkout 등록카드 결제는 계정·환경·소유권·서버 금액·주문별 청구 원장으로 보호한다", () => {
  ...
  assert.match(action, /canClaimBillingChargeAttempt/);
  assert.match(action, /requestAttempts:\s*expected\.expectedAttempt/);
  assert.match(action, /laonpayChargeId:\s*null/);
  assert.match(action, /providerPaymentId:\s*null/);
  assert.match(action, /claim\.idempotencyKey/);
  assert.match(action, /billing-charge:\$\{chargePrepared\.charge\.id\}/);
  assert.match(action, /chargeResult\.outcome === "UNKNOWN"/);
  assert.match(action, /markFirstBillingAttemptUnknown/);
  assert.match(
    action,
    /status:\s*"REQUESTING"[\s\S]*requestAttempts:\s*1[\s\S]*data:\s*\{[\s\S]*status:\s*"UNKNOWN"[\s\S]*RESULT_UNCONFIRMED/,
  );
  assert.match(
    action,
    /const markedUnknown = await markFirstBillingAttemptUnknown[\s\S]*if \(markedUnknown\)[\s\S]*expectedAttempt:\s*1[\s\S]*chargePaymentMethod/,
  );
  ...
  assert.match(
    action,
    /status:\s*"REQUESTING"[\s\S]*requestAttempts:\s*0[\s\S]*laonpayChargeId:\s*null[\s\S]*providerPaymentId:\s*null/,
  );
  assert.match(action, /closeLocalBillingChargeAndOrder/);
...
test("두 원장 tx 사이 가격 변경은 외부 미호출 charge와 주문을 같은 tx에서 종료한다", () => {
  ...
  assert.match(
    mismatchBranch,
    /closeLocalBillingChargeAndOrder\([\s\S]*"LOCAL_AMOUNT_CHANGED"/,
  );
  const helper = action.slice(
    action.indexOf("async function closeLocalBillingChargeAndOrder"),
    action.indexOf("async function closeBillingOrderWithoutCharge"),
  );
  assert.match(helper, /shopBillingCharge\.updateMany/);
  assert.match(helper, /shopOrder\.updateMany/);
  assert.match(helper, /closedCharge\.count !== 1/);
  assert.match(helper, /closedOrder\.count !== 1/);
});
...
test("등록카드 주문 취소는 부분금액 없이 LAONPAY 관리자 취소 요청을 한 번만 생성한다", () => {
  ...
  const section = action.slice(
    action.indexOf("export async function requestCancelAction"),
    action.indexOf("// ── LAONPAY 등록카드 취소 상태 대사"),
  );

  assert.match(section, /shopBillingCancelRequest\.create/);
  assert.match(section, /requestSentAt:\s*null/);
  assert.match(section, /data:\s*\{\s*requestSentAt:\s*new Date\(\)\s*\}/);
  assert.match(section, /createCancelRequest\(/);
  assert.doesNotMatch(section, /createCancelRequest\([\s\S]{0,300}\bamount\b/);
  assert.match(section, /result\.data\.charge\.status === "CANCEL_REQUESTED"/);
  assert.match(
    section,
    /result\.data\.charge\.status === "PAID"[\s\S]*result\.data\.cancelRequest\.status === "REJECTED"/,
  );
  assert.match(section, /const rejected =\s*result\.ok &&/);
  assert.doesNotMatch(section, /result\.outcome === "REJECTED"/);
  assert.match(section, /status:\s*rejected \? "REJECTED" : "UNKNOWN"/);
  assert.match(section, /reason:\s*result\.data\.cancelRequest\.reason/);
  assert.match(section, /cancelReason:\s*result\.data\.cancelRequest\.reason/);
  assert.match(section, /billing-method:\$\{prepared\.charge\.paymentMethodId\}/);
});
...
test("등록 대사는 hosted URL을 같은 key로 한 번만 회수하고 terminal 상태를 되돌리지 않는다", () => {
  ...
  assert.match(action, /const featureEnabled = isBillingIntegrationEnabled\(user\.email\)/);
  assert.match(action, /!isBillingReconciliationEnabled\(user\.email\)/);
  assert.match(
    action,
    /!featureEnabled[\s\S]*prepared\.registration\.requestAttempts === 0[\s\S]*!prepared\.registration\.laonpayRegistrationId/,
  );
  assert.match(
    action,
    /upsertOwnedBillingPaymentMethod\(\s*tx,\s*input\.userId,\s*remote\.paymentMethod/,
  );
  assert.match(
    returnRoute,
    /upsertOwnedBillingPaymentMethod\(\s*tx,\s*user\.id,\s*remote\.paymentMethod/,
  );
...
test("등록카드 결제 상태조회는 provider ID가 있으면 GET, 없으면 남은 동일-body 대사 POST만 사용한다", () => {
  ...
  assert.match(action, /canClaimBillingChargeAttempt\(/);
  assert.match(action, /canRecoverStaleBillingChargeRequest\(latestCharge\)/);
  assert.match(
    action,
    /status:\s*"REQUESTING"[\s\S]*requestAttempts:\s*1[\s\S]*status:\s*"PENDING"/,
  );
  assert.match(
    action,
    /updatedAt:\s*latestCharge\.updatedAt[\s\S]*status:\s*"UNKNOWN"[\s\S]*RESULT_UNCONFIRMED/,
  );
  assert.match(action, /requestAttempts:\s*claimableCharge\.requestAttempts/);
  assert.match(action, /laonpayChargeId:\s*null/);
  assert.match(action, /providerPaymentId:\s*null/);
  assert.match(action, /billing-charge:\$\{prepared\.charge\.id\}/);
  assert.match(action, /requestFingerprint !== charge\.requestFingerprint/);
  assert.match(
    action,
    /localRequestChanged && isProvablyLocalBillingCharge\(charge\)[\s\S]*shopBillingCharge\.updateMany[\s\S]*shopOrder\.updateMany/,
  );
  assert.match(
    action,
    /const reconciliationRejected =[\s\S]*!prepared\.charge\.laonpayChargeId[\s\S]*result\.outcome === "REJECTED"/,
  );
  assert.match(
    action,
    /reconciliationRejected \? \{ requestAttempts: 2 \} : \{\}/,
  );
  assert.match(action, /LAONPAY_BILLING_PROCESSING_MARKER/);
  assert.match(action, /RESULT_ID_MISMATCH/);
});

test("feature kill switch 중 상태조회는 최초 청구를 시작하지 않고 기존 대사만 허용한다", () => {
  const action = source("app/order/actions.ts");
  const refresh = action.slice(
    action.indexOf("export async function refreshBillingChargeStatusAction"),
  );

  assert.match(refresh, /const featureEnabled = isBillingIntegrationEnabled\(user\.email\)/);
  assert.match(
    refresh,
    /canClaimBillingChargeAttempt\([\s\S]*claimableCharge\.requestAttempts,\s*featureEnabled/,
  );
  assert.match(
    refresh,
    /!featureEnabled[\s\S]*claimableCharge\.status === "REQUESTING"[\s\S]*requestAttempts === 0[\s\S]*status: "PENDING"/,
  );
});
...
test("실 WEBFEP 수기결제와 심사 계정 시연 경로를 분리한다", () => {
  ...
  const liveManualSection = actions.slice(
    actions.indexOf("// ── 수기결제(구인증) — KSNET WEBFEP"),
    actions.indexOf("const base = process.env.SHOP_APP_URL"),
  );

  assert.match(page, /resolveManualPaymentMode\([\s\S]*user\.email[\s\S]*isKspayRestLiveEnabled\(\)/);
  assert.match(form, /manualPaymentMode === "review-demo" \? "manual_demo" : "manual"/);
  assert.match(actions, /manualPaymentMode !== "live"[\s\S]*MANUAL_PAYMENT_DISABLED_MESSAGE/);
  assert.match(actions, /manualPaymentMode !== "review-demo"/);
  assert.match(actions, /d\.method === "manual_demo"[\s\S]*실제 WEBFEP\/KSNET 호출과 TID 생성은 없다/);
  assert.doesNotMatch(liveManualSection, /result === null[\s\S]{0,300}status:\s*"PAID"/);

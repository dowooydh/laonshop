...
test("등록카드 취소 상태는 GET만 사용하고 CANCELED를 세 원장에 원자 반영한다", () => {
  const action = source("app/order/actions.ts");
  ...
  assert.match(section, /remoteStatus === "CANCELED"/);
  assert.match(
    section,
...
test("취소 응답 유실은 GET CANCEL_REQUESTED/CANCELED만 복구하고 PAID를 거절로 추론하지 않는다", () => {
  const action = source("app/order/actions.ts");
  ...
  assert.match(section, /remoteStatus === "CANCEL_REQUESTED"/);
  assert.match(
    section,
    /uncertainRequest[\s\S]*status:\s*"REQUESTED"[\s\S]*status:\s*"CANCEL_REQUESTED"/,
  );
  assert.match(section, /GET charge만으로 PAID를 취소 거절로 해석할 수 없다/);
  assert.doesNotMatch(section, /remoteStatus === "PAID"[\s\S]{0,500}status:\s*"REJECTED"/);

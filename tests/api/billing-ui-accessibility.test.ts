...
test("취소 신청과 공용 오류 UI는 중복 입력·지속 상태·스크린리더 경계를 제공한다", () => {
  ...
  assert.match(
    cancelForm,
    /if \(res\.ok\) \{[\s\S]*reloading = true;[\s\S]*window\.location\.reload\(\);[\s\S]*return;/,
  );
  assert.match(cancelForm, /if \(!keepLocked && !reloading\) submittingRef\.current = false/);
  assert.match(cancelForm, /if \(!reloading\) setPending\(false\)/);
  assert.doesNotMatch(cancelForm, /router\.refresh\(\)/);
  assert.doesNotMatch(cancelForm, /useRouter/);

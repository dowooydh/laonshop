...
test("상세컷 버전 URL은 Next 이미지 최적화 허용 목록과 일치한다", () => {
  ...
  assert.ok(
    nextConfig.images?.localPatterns?.some(
      (pattern) => pattern.pathname === "/products/gallery/**" && pattern.search === "",
    ),
  );
});

test("큐레이션 상품은 외부 대표컷 대신 같은 SKU의 독립 5컷 갤러리를 사용한다", () => {
  const product = {
    name: "오버핏 반팔 티셔츠",
    category: "상의",
    gender: "men",
    imageUrl:
      "https://images.unsplash.com/photo-1714249090984-cbc595b0f3b8?w=1200&h=1500&q=85&auto=format&fit=crop&crop=faces%2Centropy",
  };

  const images = getProductDetailImages(product);
  assert.equal(productDetailSlug(product), "p-1acjf79");
  assert.equal(images.length, 5);
  assert.deepEqual(
    images.map((image) => image.src),
    Array.from({ length: 5 }, (_, index) =>
      `/products/gallery/b01/p-1acjf79/${String(index + 1).padStart(2, "0")}.webp`,
    ),
  );
  assert.equal(safeProductImageUrl(product.imageUrl), "/products/gallery/b01/p-1acjf79/01.webp");
});
...
test("재생성된 로컬 상세컷 URL을 저장 이미지로 허용한다", () => {
  ...
  assert.equal(
    safeProductImageUrl("/products/gallery/b01/p-1acjf79/01.webp?stale=1"),
    "/products/gallery/b01/p-1acjf79/01.webp",
  );
  assert.equal(safeProductImageUrl("/products/gallery/b01/p-1acjf79/06.webp"), null);
  assert.equal(safeProductImageUrl("/products/gallery/b99/p-dead/01.webp"), null);
  assert.equal(safeProductImageUrl("/products/gallery/b01/p-1acjf79/05.webp"), "/products/gallery/b01/p-1acjf79/05.webp");

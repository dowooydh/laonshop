...
- 구매조건 동의
- 서버 금액 재계산
- 상품별 합산 재고 검증, 허용 사이즈 검증, 동시 재고 예약
- PENDING 주문 생성
- 다중 탭·재전송 주문 생성 멱등성
- KSPAY 폼 필드
- callback/result 상태 전이, 서명·주문 상관관계, TLS
- result 병렬 호출 성공 우선·단일 승인
- PG timeout/503 및 script 로드 실패 복구
- 영수증 링크
- 취소/반품 신청 접수
...
- 모바일/데스크톱
- 320/390/412px·200% 글자 확대
- 텍스트 겹침, 색 대비, 터치 타깃, focus 기본 동작
